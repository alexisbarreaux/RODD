...
function distance(i::Int64,j::Int64, n::Int64)::Float64
    i_line, i_col = div(i -1,n) + 1, (i-1)%n + 1
    j_line, j_col = div(j - 1,n) + 1, (j-1)%n + 1
...
function dinkelbach(inputFile::String="data", showResult::Bool= false, silent::Bool=true)::Any
    ...
    while true
        ...
        ##### Objective #####
        @objective(model, Min, sum(y[i,j]*distance(i,j,n) for i in 1:numberOfSites for j in 1:numberOfSites if i!=j)
...
function dinkelbachRandom(n::Int64=10, showResult::Bool= false, silent::Bool=true)::Any
    """
    n : grid edge size
    """
    start = time()
    c, Amin, Amax, B, lambda = generateCost(n)
    numberOfSites = n*n
